function varargout = get_hard_classprofiles(EOFs,XR,Xref,mix,LABELS,WEIGHTS,varargin)
% get_hard_classprofiles Compute representative class profiles given a GMM and a dataset (hard PDF)
%
% [MS PRC MM, PDFs] = get_hard_classprofiles(EOFS,XR,XREF,MIX,LABELS,WEIGHTS,[PAR,VAL])
%	Compute representative class profiles from a GMM and a dataset using hard classification
% statistics, i.e. we use only profiles attributed to a class to compute representative profiles.
%
% Inputs:
...
%	PRC[3,K,Nz]: Empirical class percentiles (5/50/95) from profiles hardly classified.
...
% 		PDFS.sclass [Nz,K,n]: The empirical model PDF at each level for each class (computed using hardly classified profiles)
...
% Created: 2015-09-10 (G. Maze, Ifremer, Laboratoire d'Océanographie Physique et Spatiale)
...
for iz = 1 : Nz
	...
	for ik = 1 : K
		ip = find(oLABELS==ik); % Profiles attrbiuted to this class
		ikmix = unique(LABELS(ip)); % Class id in the mix structure (ikmix ~= ik if oLABELS ~= LABELS)

		%-- Recompose profiles from this class:
		ds = EOFs(iz,lcomp)*XR(lcomp,ip) + Xref(iz); % Recompose data at this level for this class from EOF compression
		...
		Cmean_samp(ik,iz)  = wmean(WEIGHTS(ip,ikmix),ds);
		Cstd_samp(ik,iz)   = wstd(WEIGHTS(ip,ikmix),ds'); 
		Cskew_samp(ik,iz)  = wskewness(WEIGHTS(ip,ikmix),ds'); 
		
		%-- Compute 5%/50%/95% percentile of profiles for this class:
		C05_samp(ik,iz)   = wprctile(ds, 5,WEIGHTS(ip,ikmix));
		Cmedi_samp(ik,iz) = wprctile(ds,50,WEIGHTS(ip,ikmix));
		C95_samp(ik,iz)   = wprctile(ds,95,WEIGHTS(ip,ikmix));
...
% 		PDFS.recomp [Nz,n]:   The PDF at each level from the sum of priors weighted pdfs of each class
	...
	PDFS.readme = 'Only profiles attributed to each class were used';
	varargout(4) = {PDFS};
end% if 

end %functiongmmargo_1param_classprofiles