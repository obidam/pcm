function [mix,post, options, errlog] = gmmLINem(mix,x,W,indZeroLinCoef, options)
...
% $$$     A = max((wAtA(:)-wAtA2(:)).^2)
    ...
    if debug
      ...
      subplot(311);plot(x(ind,1),x(ind,2),'b*');
      subplot(312);hold on;
      plot(W(j).w(ind,1),x(ind,1),'r*');
      plot(W(j).w(ind,1),xpred(:,1),'b*');
      subplot(313);hold on;
      plot(W(j).w(ind,1),x(ind,2),'r*');
      plot(W(j).w(ind,1),xpred(:,2),'b*');
      pause;
    end
...
options(8) = -sum(log(gmmLIN2prob(mix, x, W)));
if (display >= 0)
  disp(maxitmess);
end
