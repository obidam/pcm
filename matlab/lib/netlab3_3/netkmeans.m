function [centres, options, post, errlog, cvgcause] = netkmeans(centres, data, options)
...
%		fprintf(1, 'Cycle %4d  Error %11.6f\n', n, e);
		if n>1
			disp(sprintf('Cycle %4d: Centres (%11.6f) and error (%11.6f) convergence levels',n,max(max(abs(centres - old_centres))),abs(old_e - e)));		
		end% if 
	end% if 
	...
	if n > 1
		...
		if max(max(abs(centres - old_centres))) < options(2) & abs(old_e - e) < options(3)
			options(8) = e;
			cvgcause = sprintf('Centres (%11.6f) and error (%11.6f) convergence levels reached',max(max(abs(centres - old_centres))),abs(old_e - e));		
			return;
		end% if 
	end% if 
	old_e = e;
...
% iterations.
options(8) = e;
cvgcause = 'Maximum nb of iterations reached';
if (options(1) >= 0)
	disp(maxitmess);	
end
