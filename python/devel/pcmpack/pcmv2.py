...
The goal is to create a class that that behave like any scikit method and can
be used into a Pipeline. 
That's why the terminology is different from the Matlab toolbox.

The workflow is taken from the list of methods used in the Matlab implementation:
    - the PCM is defined with a number of class and vertical depth axis
    - to fit or predict any data, the collection is:
        - interpolated on the PCM axis if necessary
        - normalized or centered along the collection , level by level
        - ...

The python class is an instance with the PCM Matlab structure returned by a 
pcmsave/pcmload    
...
# Decorators:
def deprec_mess(fct):
    print "Version 2 is the most stable PCM version"
    return fct
...
class PCM:
    ...
    def preprocessing(self, X, Z):
        """"Pre-process data for classification
            ...
            Returns
            -------
            X : array-like, shape (N_p=n_sample, n_reduced_scaled_interpolated_features)
